...
if [ "$VG" == "ON" ] ; then
    CMFLAGS="$CMFLAGS -DC4STL_VALGRIND=ON -DC4STL_VALGRIND_SGCHECK=OFF" # FIXME SGCHECK should be 1
elif [ "$VG" == "OFF" ] ; then
    CMFLAGS="$CMFLAGS -DC4STL_VALGRIND=OFF -DC4STL_VALGRIND_SGCHECK=OFF"
fi
